...
- **クリップボード画像の保存**: 画像をコピーして `CmdOrCtrl+V` を押すと、Markdownファイルと同じ階層の `images` ディレクトリにPNGとして保存し、画像を参照するMarkdown記法をクリップボードにコピーします。スライドの幅・高さを超える画像は、縦横比と透過を保ち、確認ダイアログなしでスライド内に収まるサイズまで縮小します。上限は表示中のデッキのスライドサイズ（標準1280×720、`size: 4:3` は960×720など）で、小さい画像は拡大しません。スライドサイズがまだ取得できない場合は1280×720を使います。
